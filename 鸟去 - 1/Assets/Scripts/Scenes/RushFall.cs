...
public class RushFall : MonoBehaviour
{
    ...
    [SerializeField]
    float InternalMin = 0.5f, InternalMax = 1.5f;
    float CurInternal;
    ...
    void Update()
    {
        if(collided)
        {
            ...
            if (CurInternal <= 0)
            {
                ...
                GameObject fallStone = (GameObject)GameObject.Instantiate(Resources.Load("FallStones\\FallStone_1"), transform);
                fallStone.GetComponent<Rigidbody2D>().angularVelocity = Random.Range(-180f, 180f);
                fallStone.GetComponent<Rigidbody2D>().velocity = new Vector2(0, Random.Range(0, -25));
                fallStone.transform.localScale = Random.Range(/*0.6f, 1f*/0.4f, 0.6f) * Vector3.one;
                fallStone.transform.position = new Vector2(Player.transform.position.x + Random.Range(-Global.CamWidth * 0.3f, Global.CamWidth * 1.8f), Global.BGHight * 1.5f);
                mYBackInTime.addToBackInTime(fallStone);
            }
            if(transform.childCount > 20)
            {
                GameObject rock = transform.GetChild(0).gameObject;
                mYBackInTime.removeFromBackInTime(rock);
                Destroy(rock);
            }
        }
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(collision.gameObject.tag == "Player" && !collided)
        {
            ...
            mYBackInTime.addToBackInTime(Player, false);   // 变得可回溯了
        }
    }
