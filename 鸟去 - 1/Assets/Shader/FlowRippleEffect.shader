Shader "Hidden/Flow Ripple Effect"
{
    Properties
    {
        ...
        _Rect("Rect", Vector) = (0, 0, 0, 0)    // (Xmin, Xmax, Ymin, Ymax)
    }
    CGINCLUDE
    ...
    float wave(float2 position, float2 origin, float time) //当前点位置, 出发点位置, 时间
    {
        ...
        //------- add
        // 矩形水波
        if(position.x < origin.x - _Rect.x || position.x > origin.x + _Rect.y
            //position.y < origin.y - _Rect.z || position.y > origin.y + _Rect.w
            )
        {
            return 0;
        }
        ...
        // // 扇形水波
        // if(dot(position - origin, _Direction)/d < 0.95)
        // {
        //     return 0;
        // }

        //------- add
		return (tex2D(_GradTex, float2(t, 0)).a - 0.5f) * 1.5f;
    }
