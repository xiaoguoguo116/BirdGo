 ...
 * 指定区域和方向发出矩形水波
 */
#define MULTI_RIPPLE  // 同时多个波
using UnityEngine;
using System.Collections;

public class FlowRippleEffect : MonoBehaviour
{
    public Camera c;
    ...
    [Range(0.01f, 1.0f)]
    public float reflectionStrength = 0.1f;//反射效果强度，可以理解为涟漪的阴影
    [Range(0.0f, 3.0f)]
    public float waveSpeed = 1.25f;//传播速度
    [SerializeField]
    Shader shader;
    Vector2 direction;  // 只有上下左右4个方向
    Vector4 rect;       // (Xmin, Xmax, Ymin, Ymax)
    class Droplet
    {
        Vector2 position;
    ...
    void UpdateShaderParameters()//更新shader参数
    {
        ...
        //material.SetVector("_Direction", new Vector4(direction.x, direction.y, 0, 0));
    ...
    public void Emit(Vector2 start, Vector2 direction, Vector4 rect)// call this to emit a ripple
    {
        this.direction = direction;
        this.rect = rect;
        droplets[dropCount++ % droplets.Length].Reset(start);
    }
}
