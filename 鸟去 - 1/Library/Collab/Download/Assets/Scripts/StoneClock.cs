...
public class StoneClock : MonoBehaviour {
    ...
    public void OnBackInTime(int length, int Length)
    {
        ...
        Outside.transform.rotation = Quaternion.Euler(0, 0, - (float)length / Length * 270 + 78.64f);  // 78.64f是初始角度
    ...
    IEnumerator BackInTime(int length, int Length)
    {
        while (length-- > 0)
        {
            Outside.transform.Rotate(0, 0, 270f / Length);
