...
public class CameraFollowNetwork : MonoBehaviour
{
    ...
    bool isRight = false;
    //}}
    bool isBorader = false;//到达活动边界判断变量
    float Gblength  = 11.8f;//乌龟与两边相机跟随最短距离,半个屏幕
    void ChangeLive() {
        islive = true;
    }
...
//        Global.Player = Global.Player;
    ...
    void Update()
    {
        ...
        if ((Global.Player.transform.position.x - GameObject.Find("GameWallLeft").transform.position.x) <= Gblength)
        {
            isBorader = true;
            if (!transform.parent)
                transform.parent = GameObject.Find("GameWall").transform;
            return;
        }
        if ((GameObject.Find("GameWallRight").transform.position.x - Global.Player.transform.position.x) <= Gblength)
        {
            isBorader = true;
            if(!transform.parent)
                transform.parent = GameObject.Find("GameWall").transform;
    ...
    void LateUpdate()
    {
        ...
        if (IsAutoScroll)
        {
            ...
            if (islive&&(Global.Player.transform.position.x <= leftBorader - deadLine))
            {
                islive = false;
                //Debug.Log("你挂了");
                //PlayerPrefs.SetInt("isDeadLastTime", 1);
                Global.Player.GetComponent<PlayerEventsNetwork>().m_life = 0;
                Invoke("ChangeLive", 0.5f);
            }
        }
        else
        {
