...
public class FingerTestNetwork : MonoBehaviour
{
    ...
    public void Swipe(Vector2 move)
    {
        ...
        Global.Player.GetComponent<PlayerEventsNetwork>().CmdForce(force);
        ...
        foreach (GameObject os in oss)
        {
          ...
          && (os.transform.position.x > leftBorder))

                {
                    ...
                    force *= rb.mass;    // 乌贼与龟同速的力（龟的mass是1）
                    float k = 0.5f;    // 乌贼的速度是龟的k倍
                    os.GetComponent<IDrawable>().DrawForce(force * k);
    ...
    void Tap(DiscreteGesture gesture)
    {
        ...
        else if (ob && ob.tag == "Player" && Global.SceneEvent.GetCurMystery().GetComponent<MYBackInTimeNetwork>())
        {
            foreach (var btn in GameManager.SkillButton)
            {
